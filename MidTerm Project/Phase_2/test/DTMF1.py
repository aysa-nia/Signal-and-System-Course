...
#az haman tabe ke dar phase aval estefade karde bodim dobare estefade mikonim
#montaha chon on baray yek klid zade shode bood bayad file audio vorodi ro slice konim betori ke 
#dar har ghesmatesh yek klid zade shode bashe va ono vorodi bedim be tabe DTMF va bebinim che klidi zade shode 
#baray slicing farz shode feshrofdan har klid hadaksar 300 milisecons tool bekshad albate in farz 
#daghig va dorost nist chon momkene file sooti vorodi ma ingoone nabashad
def Slicing(signal, rate):
    result = []   
    for i in range ( 0 , len(signal) - int(rate * 0.3) ,int(rate) ):         
        x = []
        if( signal[i] != 0 ):
            for j in range(0 , int(rate * 0.3)):
                x.append(signal[i + j]);            
            c = DTMF_First(x , rate)
            if(c!=''):
                result.append(c)
